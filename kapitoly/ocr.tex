...
To test how is the model successful various evaluation metrics were created. The simplest that comes to a mind is compare the word in the image with the predicted one. This approach is straightforward and it is a useful for long text ,for example scanned books or documents. However, used on scene text or generally images with sparse text that does  not contain sentences, it has some drawbacks. 